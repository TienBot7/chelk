...
const isMobileThankYouScreen = () => window.innerWidth <= 500
...
async function loadVfxModule(timeoutMs = 3000, attempts = 3) {
  ...
  console.warn('[thank-you] VFX module failed to load after retries')
  return null
}
...
function preloadThankYouAudio() {
  try {
    ...
    preloadAudio.volume = THANK_YOU_SOUND_VOLUME
    preloadAudio.load()
  } catch (e) {}
}
...
let decodedChekBuffer = null
let audioUnlocked = false
let lastScreenTapTime = 0
const THANK_YOU_SOUND_VOLUME = 0.54
...
const getButtonAudio = (resetCurrentTime = true) => {
  ...
  if (!buttonAudio.src) buttonAudio.src = './audio/chelk.mp3'
  if (buttonAudio.readyState === 0) {
    try { buttonAudio.load() } catch (e) {}
  }
  return buttonAudio
}
...
const playCheklSound = () => {
  try {
    if (audioContext && decodedChekBuffer) {
      ...
      gain.gain.value = THANK_YOU_SOUND_VOLUME
      src.connect(gain)
...
const playScreenTapSound = () => {
  const now = Date.now()
  if (now - lastScreenTapTime < 180) return
  lastScreenTapTime = now
  playCheklSound()
}

function initCheklWebAudio() {
  ...
  fetch('./audio/chelk.mp3').then((r) => r.arrayBuffer()).then((ab) => {
    ...
    try {
      const dec = audioContext.decodeAudioData(ab, (buf) => { decodedChekBuffer = buf })
      if (dec && typeof dec.then === 'function') dec.then((buf) => { decodedChekBuffer = buf }).catch(() => {})
    } catch (err) {
      try { audioContext.decodeAudioData(ab).then((buf) => { decodedChekBuffer = buf }).catch(() => {}) } catch (e) {}
    }
  }).catch(() => {})
