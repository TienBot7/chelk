  ...
  loadLottieLib()
    .then((lottie) => {
      ...
      const updateFrame = () => {
        ...
        const svgElement = lottieContainerEl.querySelector('svg')
        if (svgElement) {
          const marginBottom = 80 * (1 - Math.min(1, scaleFrame / 15))
          svgElement.style.marginBottom = `${marginBottom}px`
        }
        ...
        if (overlayIndicator) {
          const fadeDistance = Math.max(64, scrollArea.clientHeight * 0.12)
          const indicatorOpacity = Math.max(0, Math.min(1, 1 - scrollArea.scrollTop / fadeDistance))
      ...
      const revealLottieOverlay = () => {
        console.log('✨ Revealing lottie overlay');
      ...
      // Add event listeners (may not all fire, that's why we have the setTimeout below)
      if (lottieAnimation) {
        ...
        lottieAnimation.addEventListener('ready', revealLottieOverlay)
      }

      // Guarantee overlay will be shown after animation loads, even if events don't fire
      setTimeout(() => {
        console.log('🎬 Checking lottie state:', { 
          hasAnimation: !!lottieAnimation,
          hasFrames: lottieAnimation && lottieAnimation.totalFrames > 0,
          overlayHasShow: overlay.classList.contains('show')
        });
        if (lottieAnimation && !overlay.classList.contains('show')) {
          console.warn('⚠️ Lottie events did not trigger reveal, forcing it now');
          revealLottieOverlay()
        }
      }, 300)
    })
